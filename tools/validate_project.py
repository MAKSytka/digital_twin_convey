...
ROWS = 14
COLS = 4
EXPECTED_CELLS = {f"cell_r{row:02d}_c{col:02d}" for row in range(ROWS) for col in range(COLS)}
EXPECTED_COMMAND_TOPICS = {f"/singulator/cell/r{row:02d}_c{col:02d}/cmd_vel" for row in range(ROWS) for col in range(COLS)}
...
def validate_world() -> None:
    ...
    topics = {element.text.strip() for element in world.findall(".//velocity_topic") if element.text}
    ...
    friction_pairs = {(float(ode.findtext("mu")), float(ode.findtext("mu2"))) for ode in world.findall(".//surface/friction/ode")}
...
def validate_matrix_config() -> None:
    ...
    expected = {
        "rows": 14, "cols": 4, "active_cell_length_m": 0.360,
        "active_cell_width_m": 0.175, "longitudinal_gap_m": 0.020,
        "transverse_gap_m": 0.020, "pitch_x_m": 0.380,
        "pitch_y_m": 0.195, "total_length_m": 5.300, "total_width_m": 0.760,
    }
...
def validate_bridges() -> None:
    files = [
        BRIDGE_DIR / "bridge_rows_00_03.yaml", BRIDGE_DIR / "bridge_rows_04_07.yaml",
        BRIDGE_DIR / "bridge_rows_08_11.yaml", BRIDGE_DIR / "bridge_rows_12_13.yaml",
...
def validate_python_package_manifests() -> None:
    ...
    for path in packages:
        ...
        if package_root.findtext("./export/build_type") != "ament_python":
            fail(f"{path}: build_type must be ament_python")
        invalid = [element.text for element in package_root.findall("buildtool_depend") if element.text == "ament_python"]
        if invalid:
            fail(f"{path}: ament_python is a build type, not a rosdep dependency")


def validate_interfaces() -> None:
    matrix_command = (ROOT / "src/singulator_interfaces/msg/MatrixCommand.msg").read_text(encoding="utf-8")
    for line in ("uint16 rows", "uint16 cols", "float32[] target_speed_mps"):
...
def validate_clean_tree() -> None:
    ...
    if forbidden:
        fail(f"Forbidden generated or backup paths are tracked: {sorted(forbidden)}")


def main() -> int:
    checks = (
        ("world", validate_world), ("matrix config", validate_matrix_config),
        ("bridge coverage", validate_bridges), ("2 m/s defaults", validate_speed_defaults),
        ("Python package manifests", validate_python_package_manifests),
        ("interfaces", validate_interfaces), ("Python syntax", validate_python),
