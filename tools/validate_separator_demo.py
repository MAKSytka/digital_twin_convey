...
def main() -> None:
    ...
    require(len(includes) == 11, "Expected 11 continuous rollers")
    xs = sorted(values(node.findtext("pose"))[0] for node in includes)
    pitch = min(right - left for left, right in zip(xs, xs[1:]))
    require(abs(pitch - 0.120) < 1e-6, "Roller pitch must be 120 mm")

    rotor = roller.find(".//link[@name='rotor']")
    require(rotor is not None, "Rotor link is missing")
    require(abs(values(rotor.findtext("pose"))[2] - 0.055) < 1e-9, "Roller axis Z must be 55 mm")
    ...
    require(abs(radius - 0.025) < 1e-9, "Roller radius must be 25 mm")
    require(abs(length - 2.480) < 1e-9, "Roller length must be 2.480 m")
    require(radius == visual_radius and length == visual_length, "Roller visual and collision differ")
    require(
        not any(
    ...
    require(abs(opening - 0.070) < 1e-6, "Longitudinal opening must be 70 mm")

    joint = roller.find(".//joint[@name='shaft_joint'][@type='revolute']")
    require(joint is not None, "Roller revolute joint is missing")
    require(values(joint.findtext("./axis/xyz")) == [0.0, 1.0, 0.0], "Roller axis must be Y")
    require(
        roller.find(".//plugin[@filename='gz-sim-joint-controller-system']") is not None,
    ...
    require(
        abs(box_size(reject_transfer, "reject_transfer_belt_collision")[1] - 2.5) < 1e-9,
    ...
    for suffix in ("infeed", "screen", "accepted", "reject_transfer", "reject"):
        require(
    ...
    require("angular_speed = surface_speed / radius" in controller, "Surface-speed calculation is missing")
    require("rpm = angular_speed * 60.0 / (2.0 * math.pi)" in controller, "RPM calculation is missing")
    require(abs(2.0 / radius - 80.0) < 1e-9, "2 m/s must produce 80 rad/s")

    base_spawner = SPAWNER_BASE.read_text(encoding="utf-8")
    require(
        re.search(r"declare_parameter\(\s*[\"']small_item_probability[\"']", base_spawner) is not None,
    ...
    for profile in ("micro_parcel", "long_narrow", "flat_strip", "tall_slender", "near_cutoff"):
        require(f'"{profile}"' in spawner, f"Lower profile is missing: {profile}")

    setup = SIM_SETUP.read_text(encoding="utf-8")
    require(
        "separator_demo_spawner = singulator_sim.separator_continuous_roller_spawner:main" in setup,
