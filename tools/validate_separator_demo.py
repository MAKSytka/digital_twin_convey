...
EXPECTED_ROLLERS = 11
EXPECTED_PITCH_M = 0.130
EXPECTED_RADIUS_M = 0.025
EXPECTED_OPENING_M = 0.080
...
def main() -> None:
    ...
    require(
        ...
        "Roller pitch must be 130 mm for every adjacent pair",
    )
    pitch = pitches[0]
    ...
    require(
        ...
        "Longitudinal opening must be 80 mm",
    )
    ...
    require(
        abs(transfer_size[0] - 1.5) < 1e-9,
        "Lower transfer belt must be 1.5 m long for the wider screen",
    )
    require(
    ...
    print(
        ...
        "lower_cutoff=0.070 m, upper_safety=0.090 m, "
