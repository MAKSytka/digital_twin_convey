...
Роликовый экран состоит из 11 сплошных поперечных валов длиной 2,480 м. Межосевой шаг равен 150 мм, диаметр ролика — 50 мм, поэтому чистое продольное отверстие составляет 100 мм.

Классификация выполняется по продольной опорной проекции `projection_x`. Товары с `projection_x < 70 мм` ожидаются на нижней ветви, а безопасный верхний класс начинается с `projection_x >= 110 мм`. Физическое отверстие даёт 30 мм запаса нижнему классу и 10 мм запаса до верхнего класса. Пограничный диапазон 70–110 мм исключён из release-потока. Фактический маршрут определяется по потоку поз Gazebo и сравнивается с ожидаемым.
