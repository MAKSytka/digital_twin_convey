...
| Полноширинный инфид-сепаратор | требуется повторный запуск после увеличения шага до 130 мм | версия с шагом 120 мм подтверждена 2026-08-01 | повторная проверка |
...
- верхний безопасный класс начинается с `projection_x >= 0,090 м`;
- вероятность нижнего класса 0,70.

Выбор шага 130 мм создаёт по 10 мм физического запаса относительно обоих программных классов: нижняя граница заканчивается на 70 мм, отверстие равно 80 мм, верхний безопасный класс начинается с 90 мм.
...
- длинные узкие товары поперечной ориентации надёжно проходят через 80-мм продольные промежутки;
- крупные товары не проходят сквозь ролики и не проваливаются в увеличенные отверстия;
