...
| Полноширинный инфид-сепаратор | требуется повторный запуск после изменения шага и демпфирования | предыдущая контактная настройка подтверждалась до финальной правки | повторная проверка |
...
## 2. Инфид-сепаратор — повторная проверка после изменения физики
...
Выбор шага 150 мм создаёт 30 мм физического запаса для нижнего класса и 10 мм запаса до безопасного верхнего класса. Пограничный диапазон 70–110 мм исключён из release-потока.
...
- длинные узкие товары поперечной ориентации надёжно проходят через 100-мм продольные промежутки;
- товары верхнего класса с `projection_x >= 110 мм` не проваливаются;
