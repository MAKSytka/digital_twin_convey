...
### Экспериментальный контроллер V7

V7 использует неизменяемую глобальную очередь, прямое управление зазорами соседних товаров и логические идентификаторы поверх raw-ID машинного зрения. Абсолютные временные слоты и повторное создание кластеров удалены.
...
Описание: [`docs/V7_GLOBAL_QUEUE_CONTROL.md`](docs/V7_GLOBAL_QUEUE_CONTROL.md).
...
Рабочий roller-launch запускает V7-контроллер с диапазоном `1.00–3.00 м/с`, ускорением `6 м/с²` и прямым adjacent-gap управлением. Порядок товаров фиксируется один раз на входе и не пересчитывается по текущей координате `X`.

Описание параметров и методика испытаний:
[`docs/V7_GLOBAL_QUEUE_CONTROL.md`](docs/V7_GLOBAL_QUEUE_CONTROL.md).
